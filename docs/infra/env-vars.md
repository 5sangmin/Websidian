...
- 공통값은 `.env`에서 가져온다
- 서비스별로 필요한 값만 주입한다
- 이미지 버전도 가능하면 환경 변수로 분리해 명시적으로 관리한다
...
원칙:
- 기본 설정은 `application.properties` 또는 `application.yml`
- 환경별 설정은 `application-dev.properties`, `application-prod.properties` 또는 profile별 yml을 사용한다
- 비밀값/가변값은 환경 변수로 주입한다
- DB/MinIO 연결 정보는 환경 변수 우선으로 관리한다
